...
describe('FullJudgment.equals', () => {
  ...
  const bits110 = BitSequence.fromString('110');
  ...
  const fj110AllCorrect = new FullJudgment(true, bits110, [allCorrect101]);
  ...
  it('returns false if isCorrect differs', () => {
    const fj1 = fj101AllCorrect;
    const fj2 = new FullJudgment(false, new BitSequence('101'), [allCorrect101]);
    expect(fj1.equals(fj2)).to.be.false;
  });

  it('returns false if correctGuess differs', () => {
    const fj1 = fj101AllCorrect;
    const fj2 = new FullJudgment(true, new BitSequence('110'), [allCorrect101]);
    expect(fj1.equals(fj2)).to.be.false;
  });
