...
Bit correctness is expressed through pixel-art sprite swaps (`data-correctness` attribute), not CSS color values. The visual colors of those sprites are:
...
- The `fg` dark-teal (`#1e272a`) and the `bg` light-grey (`#d0d4d4`) are derived from the same teal family, giving the UI its muted, LCD-screen feel.
